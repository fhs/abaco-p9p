...
void
wininit(Window *w, Window *notused, Rectangle r)
{
	Rectangle r1, br;
...
winsettag1(Window *w)
{
	...
	i = 0;
	runemove(new+i, ((Rune[]){ ' ','D','e','l',' ','S','n','a','r','f',0}) , 10);
	i += 10;
	if(w->history.nurl){
		if(w->history.cid > 0){
			runemove(new+i, ((Rune[]){ ' ','B','a','c','k',0}), 5);
			i += 5;
		}
		if(w->history.cid < w->history.nurl-1){
			runemove(new+i, ((Rune[]){ ' ','N','e','x','t',0}), 5);
			i += 5;
		}
		if(w->page.loading){
			runemove(new+i, ((Rune[]){ ' ','S','t','o','p',0}), 5);
			i += 5;
		}
	}
	runemove(new+i, ((Rune[]){ ' ','G','e','t',0}), 4);
	i += 4;
	runemove(new+i, ((Rune[]){ ' ','|',' ',0}), 3);
	i += 3;
...
/*
	...
	if(r)
		...
		k = w->tag.rs.nr;
		if(w->page.url){
			runemove(new+i, (Rune[]){ ' ','L','o','o','k',' ',0} , 6);
			i += 6;
		}
	}
*/
...
int
winclean(Window *notused, int notused2)
{
	return TRUE;
}
