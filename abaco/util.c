...
void initfontpaths(void)
{
	...
	snprint(buf, sizeof(buf)-1, "%s/.abaco.fonts", getenv("HOME"));
...
plumbrunestr(Runestr *rs, char *attr)
{
	...
	if(plumbsendfid){
		...
		m->ndata = -1;
		i = plumbsendtofid(plumbsendfid, m);
		plumbfree(m);
	}
	return i;
}
...
ucvt(Rune* s)
{
	...
	for(i=0; i<n; i++){
		c = s[i];
		if(inclass(c, (Rune[]){'-',' ','/','$','_','@','.','!','*','\'','(',')',',','a','-','z','A','-','Z','0','-','9',0}))
			len++;
		else
			len += 3;
	}
...
execproc(void *v)
{
	...
	fd[2] = dup(2, -1);
	free(e);
	threadexecl(sync, fd, "sh", "sh", "-c", cmd, 0);
 ...
 * so we user the charset based on priority.
 */
char *
...
getimage(Cimage *ci, Rune *altr)
{
	...
	mi = ci->mi;
	if(mi == nil){
		snprint(buf, sizeof(buf), "[%S]", altr ? altr : (Rune[]){'I','M','G',0});
		r.min = Pt(0, 0);
...
*/
	...
	i2 = eallocimage(display, i->r, RGB24, 1, 0);
...
fixtext1(Item **list)
{
	...
	for(it=*list; it!=nil; it=prev->next){
		...
		if(*s){
			...
			ntext->s = s2;
			ntext->item.ascent = text->item.ascent;
			ntext->item.anchorid = text->item.anchorid;
			ntext->item.state = text->item.state&~(IFbrk|IFbrksp|IFnobrk|IFcleft|IFcright);
			ntext->item.tag = text->item.tag;
			ntext->fnt = text->fnt;
			ntext->fg = text->fg;
			ntext->ul = text->ul;
			ntext->item.next = (Item *)text->item.next;
			text->item.next = (Item *)ntext;	
		}
		free(text->s);
		text->s = s1;
    Continue:
...
addrefresh(Page *p, char *fmt, ...)
{
	...
	incref(&p->w->ref);				/* flushrefresh will decref */
