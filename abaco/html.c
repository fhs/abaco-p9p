...
static void sizeitem(Lay *notused, Item *);
...
sizetext(Lay *lay, Itext *i)
{
	...
	i->item.height = lay->font->height + 2*Space;
	i->item.width = runestringwidth(lay->font, i->s);
	i->item.width += runestringnwidth(lay->font, (Rune[]){' ',0} , 1);
}
...
{
	i->item.width = lay->width;
	i->item.height = Space + i->size + Space;
}

static
void
sizeimage(Lay *notused, Iimage *i)
{
	Cimage *ci;
	...
	if(ci->i == nil)
		return;
	i->item.width = Dx(ci->i->r) + i->border + i->hspace;
	i->item.height = Dy(ci->i->r) + i->border + i->vspace;
}

static
void
sizetextfield(Lay *notused, Iformfield *i)
{
	Formfield *ff;
	Font *f;
	int w, h;
	...
	f = getfont(WFont);
	i->item.width = runestringnwidth(f, (Rune[]){ '0',0} , 1)*w + 2*(Space+Border+Margin);
	i->item.width += Scrollsize+Scrollgap;
	i->item.height = f->height*h + 2*(Space+Border+Margin);
}

static
void
sizecheck(Lay *notused, Iformfield *i)
{
	i->item.width = Boxsize + Space;
	i->item.height = Boxsize;
}

static
void
sizebutton(Lay *notused, Iformfield *i)
{
	Font *f;
	int x;
	...
	f = getfont(WFont);
	i->item.width = runestringwidth(f, i->formfield->value) + 2*x + Space;
	i->item.height = f->height + 2*x;
}
...
sizefimage(Lay *lay, Iformfield *i)
{
	...
	sizeimage(lay, ii);
	i->item.width = ii->item.width;
	i->item.height = ii->item.height;
}

static
void
sizeselect(Lay *notused, Iformfield *i)
{
	Option *o;
	Font *f;
	int x;
	...
	for(o=i->formfield->options; o!=nil; o=o->next)
		i->item.width = max(i->item.width, runestringwidth(f, o->display));
	x = Margin + Border + Space;
	i->item.width += 2*x;
	i->item.height = f->height+2*x;
}
...
sizetable(Lay *lay, Itable *i)
{
	...
	i->item.width = i->table->totw;
	i->item.height = i->table->toth;
}
...
{
	sizeitem(lay, i->item);
	i->_item.width = i->item->width;
	i->_item.height = i->item->height;
}
...
sizespacer(Lay *lay, Ispacer *i)
{
	if(i->spkind != ISPnull){
		if(i->spkind == ISPhspace)
			i->item.width = stringnwidth(lay->font, " ", 1);
		i->item.height = lay->font->height + 2*Space;
	}
}
...
drawrule(Box *b, Page *p, Image *im)
{
	...
	r.min.y += Space;
	r.max.y -= Space;
	draw(im, r, display->black, nil, ZP);
}
...
static
void
drawnull(Box *notused, Page *notused2, Image *notused3)
{
}
...
whichtarget(Page *p, int t)
{
	...
	case FTparent:
		...
		break;
	default:
		if(runestrcmp(targetname(t),q) == 0)
			error("targetname");
...
submit(Page *p, Formfield *formfield, int subfl)
{
	...
	for(f=form->fields; f!=nil; f=f->next){
		...
		if(f->ftype==Fsubmit && (f!=formfield || !subfl))
			continue;
		if(f->value==nil || f->name==nil || runestrcmp(f->name, (Rune[]){'_','n','o','_','n','a','m','e','_','s','u','b','m','i','t','_',0})==0)
			continue;
...
static void justline1(Line *notused, int);
