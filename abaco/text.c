...
textfill(Text *t)
{
	...
	do{
		...
		frinsert(&t->Frame, rp, rp+i, t->nchars);
...
texttype(Text *t, Rune r)
{
	...
	case Kpgdown:
		n = 2*t->maxlines/3;
	case_Down:
		q0 = t->org+frcharofpt(&t->Frame, Pt(t->r.min.x, t->r.min.y+n*t->font->height));
...
textframescroll(Text *t, int dl)
{
	...
	if(dl < 0){
		...
		if(t->org+t->nchars == t->rs.nr)
			return;
		q0 = t->org+frcharofpt(&t->Frame, Pt(t->r.min.x, t->r.min.y+dl*t->font->height));
...
textsetselect(Text *t, uint q0, uint q1)
{
	...
	if(t->p1<=p0 || p1<=t->p0 || p0==p1 || t->p1==t->p0){
		...
		frdrawsel(&t->Frame, frptofchar(&t->Frame, t->p0), t->p0, t->p1, 0);
	...
	if(p0 < t->p0){
		...
		frdrawsel(&t->Frame, frptofchar(&t->Frame, p0), p0, t->p0, 1);
	}else if(p0 > t->p0){
		/* trim first part of selection */
		frdrawsel(&t->Frame, frptofchar(&t->Frame, t->p0), t->p0, p0, 0);
	}
	if(p1 > t->p1){
		/* extend selection forwards */
		frdrawsel(&t->Frame, frptofchar(&t->Frame, t->p1), t->p1, p1, 1);
	}else if(p1 < t->p1){
		/* trim last part of selection */
		frdrawsel(&t->Frame, frptofchar(&t->Frame, p1), p1, t->p1, 0);
	}
...
textsetorigin(Text *t, uint org, int exact)
{
	...
	if(a>=0 && a<t->nchars){
		...
		frdelete(&t->Frame, 0, t->nchars);
	t->org = org;
	textfill(t);
	textscrdraw(t);
	textsetselect(t, t->q0, t->q1);
	if(fixup && t->p1 > t->p0)
		frdrawsel(&t->Frame, frptofchar(&t->Frame, t->p1-1), t->p1-1, t->p1, 1);
}
